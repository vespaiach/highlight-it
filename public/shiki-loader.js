import { bundledLanguages as S, codeToHtml as E } from "https://esm.sh/shiki@3.0.0";
function $(F) {
  return {
    pre(G) {
      let q = { ...G };
...
function A() {
  ...
  if (!F || F.nodeName !== "SCRIPT") {
    let q = document.getElementsByTagName("script");
  ...
  if (F?.src)
    try {
      ...
      let K = q.searchParams.get("dark-theme") ?? "";
...
function O(F) {
  ...
  if (!G) return;
  let q = G.closest(".shiki-loader-tooltip"),
    J = G.closest(".shiki-loader-mockup-code");
  if (!J) return;
  let V = J.querySelector("pre code")?.textContent ?? "";
  if (!V) return;
  try {
  ...
  if (!q) return;
  let W = q.getAttribute("data-tip") || "Copy",
    X = j.get(q);
