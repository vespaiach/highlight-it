import { codeToHtml as E, bundledLanguages as S } from "https://esm.sh/shiki@3.0.0";
  ...
  return {
    pre(G) {
      ...
      return {
        ...
        tagName: "div",
        properties: {
          class: `${q.properties?.class || ""} shiki-loader-mockup-code`.trim(),
          style: `${q.properties?.style || ""}`.trim(),
        },
        children: [
          {
            type: "element",
            tagName: "div",
            properties: { class: "shiki-loader-language-label" },
            children: [{ type: "text", value: F }],
          },
          {
            type: "element",
            tagName: "div",
            properties: { class: "shiki-loader-tooltip", "data-tip": "Copy" },
            children: [
              {
                type: "element",
                tagName: "button",
                properties: { type: "button", class: "shiki-loader-copy-button", "data-target": "copy" },
                children: [
                  {
                    type: "element",
                    tagName: "svg",
...
import { bundledThemes as D } from "https://esm.sh/shiki@3.0.0";

var L =
    '.shiki-loader-mockup-code { border-radius: 0.5rem; direction: ltr; padding-block: 1.25rem; font-size: 0.875rem; position: relative; overflow: visible; } .shiki-loader-mockup-code:before { content: ""; opacity: 0.3; border-radius: 3.40282e38px; width: 0.75rem; height: 0.75rem; margin-bottom: 1rem; display: block; box-shadow: 1.4em 0, 2.8em 0, 4.2em 0; } .shiki-loader-mockup-code pre { padding-right: 1.25rem; padding-left: 2ch; overflow: auto; white-space: pre-wrap; display: inline-block; } .shiki-loader-language-label { position: absolute; left: 1rem; top: 0.75rem; font-size: 0.75rem; text-transform: uppercase; letter-spacing: 0.08em; opacity: 0.8; } .shiki-loader-tooltip { position: absolute; right: 0.75rem; top: 0.75rem; display: inline-flex; align-items: center; justify-content: center; z-index: 10; --tt-off: calc(100% + 0.5rem); --tt-tail: calc(100% + 1px + 0.25rem); --tw-content: attr(data-tip); } .shiki-loader-copy-button { cursor: pointer; width: 1.5rem; height: 1.5rem; background: transparent; border: none; padding: 0; display: flex; align-items: center; justify-content: center; } .shiki-loader-tooltip:hover::before { opacity: 1; --tt-pos: 0rem; } .shiki-loader-tooltip::before { border-radius: 0.25rem; text-align: center; white-space: normal; max-width: 20rem; opacity: 0; color: oklch(92% 0.004 286.32); background-color: oklch(14% 0.005 285.823); pointer-events: none; z-index: 2; content: var(--tw-content); width: max-content; padding-block: 0.25rem; padding-inline: 0.5rem; font-size: 0.875rem; line-height: 1.25; position: absolute; transform: translate(calc(var(--tt-pos, 0.25rem) - 0.25rem)) translateY(-50%); inset: 50% var(--tt-off) auto auto; } .shiki-loader-language-label { position: absolute; left: 50%; transform: translateX(-50%); top: 1.05rem; font-weight: 600; text-transform: uppercase; }',
  U = new Set(Object.keys(D));
function _() {
  const F = document.createElement("style");
  return (F.textContent = L), F;
}
function A() {
  let F = document.currentScript;
  if (!F || F.nodeName !== "SCRIPT") {
    const q = document.getElementsByTagName("script");
    for (let J = q.length - 1; J >= 0; J--) {
      const K = q[J];
      if (K.src && (K.src.includes("shiki-loader.js") || K.src.includes("index.ts"))) {
        F = K;
        break;
      }
    }
  }
  const G = { theme: "material-theme" };
  if (F?.src)
    try {
      const q = new URL(F.src),
        J = q.searchParams.get("theme") ?? "";
      if (J && U.has(J)) G.theme = J;
      const K = q.searchParams.get("dark-theme") ?? "";
      if (K && U.has(K)) G.darkTheme = K;
    } catch (q) {
      return console.error("Failed to parse script URL:", q), G;
    }
  return G;
}
var j = new WeakMap();
function O(F) {
  ...
  if (!G) return;
  const q = G.closest(".shiki-loader-tooltip"),
    J = G.closest(".shiki-loader-mockup-code");
  if (!J) return;
  const V = J.querySelector("pre code")?.textContent ?? "";
  if (!V) return;
  try {
    ...
    console.error("Failed to copy code to clipboard:", z);
  }
  if (!q) return;
  const W = q.getAttribute("data-tip") || "Copy",
    X = j.get(q);
  if (X !== void 0) window.clearTimeout(X);
  q.setAttribute("data-tip", "Copied");
  const H = window.setTimeout(() => {
    q.setAttribute("data-tip", W), j.delete(q);
  }, 2000);
  j.set(q, H);
}
var R = new Set(Object.keys(S)),
  B = !1;
async function N() {
  if (B) return;
  (B = !0), document.head.appendChild(_());
  const { theme: F, darkTheme: G } = A(),
    q = document.querySelectorAll("pre code"),
    K = (window.matchMedia?.("(prefers-color-scheme: dark)").matches && G) || F;
  for (const V of Array.from(q)) {
    const W = V,
      X = W.parentElement;
    if (!X || X.tagName !== "PRE") continue;
    let z = Array.from(W.classList).find((Y) => Y.startsWith("language-") || Y.startsWith("lang-")),
      Q = "unknown";
    if (z) {
      if (z.startsWith("language-")) Q = z.replace("language-", "");
      else if (z.startsWith("lang-")) Q = z.replace("lang-", "");
    }
    if (Q === "unknown" || !R.has(Q)) {
      console.warn("Unsupported or unknown language:", Q);
      continue;
    }
    try {
      const Y = W.textContent || "",
        w = await E(Y, { lang: Q, theme: K || "material-theme", transformers: [$(Q)] }),
        I = document.createElement("div");
      I.innerHTML = w;
      const Z = I.firstElementChild;
      if (Z) {
        const M = z || `language-${Q}`;
        if (M) Z.classList.add(M);
        X.replaceWith(Z);
      }
    } catch (Y) {
      console.error(`Failed to highlight code block with language "${Q}":`, Y);
    }
  }
  document.querySelectorAll("button[data-target='copy']").forEach((V) => {
